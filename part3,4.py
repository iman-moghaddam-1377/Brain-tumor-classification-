...
from tensorflow.keras.layers import Conv2D,add , Input, ZeroPadding2D, BatchNormalization, Activation, MaxPooling2D, Flatten, Dense, Add, DepthwiseConv2D, BatchNormalization, ReLU, GlobalAveragePooling2D, Dropout
...
def depthwise_separable_conv(inputs, filters, kernel_size=3, strides=1):
    x = tf.keras.layers.DepthwiseConv2D(kernel_size=kernel_size, strides=strides, padding='same')(inputs)
...
#model.fit(x=X_train, y=y_train, batch_size=32, epochs=30, validation_data=(X_val, y_val), callbacks=[tensorboard, checkpoint, early_stopping])
model.fit(x=X_train, y=y_train, batch_size=32, epochs=30, validation_data=(X_val, y_val), callbacks=[tensorboard, checkpoint, early_stopping])
...
for i, p in enumerate(precision):
    print(f'Precision for class {i}: {p:.4f}')
...
for i, f in enumerate(f1):
    print(f'F1-score for class {i}: {f:.4f}')   
